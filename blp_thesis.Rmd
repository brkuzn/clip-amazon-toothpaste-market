...
Prospective merger review rests on a counterfactual question: how much would prices rise if two competitors came under common ownership? The answer depends almost entirely on how closely the merging parties' products substitute for one another, and substitution patterns are, in turn, dictated by the structure the analyst imposes on demand. In nested-logit models -- still the workhorse of applied antitrust simulation -- that structure is the nest assignment. Conventional practice builds nests from coarse observable labels, most commonly brand identity or hand-picked product segments. When those labels misrepresent how consumers actually perceive products, the resulting diversion ratios, markups, and simulated price effects inherit the error.

This paper asks whether nests constructed from *unstructured product content* -- listing images and text encoded by a pre-trained deep learning model -- measure substitution better than brand labels do. The setting is well suited to the question. On Amazon, a category such as toothpaste contains hundreds of differentiated listings whose economically relevant attributes (sensitivity relief, whitening, "natural" positioning, package format) are conveyed through product images and free-form titles rather than standardised characteristics tables. I encode each product's image and listing text with CLIP (Contrastive Language-Image Pre-training; Radford et al. 2021), reduce the joint embedding to five principal components, and assign products to nests by k-means clustering in this embedding space.

The empirical laboratory is Colgate-Palmolive's acquisition of hello Products LLC, a fast-growing premium "naturally friendly" oral-care brand. The deal was announced on 23 January 2020 and completed on 31 January 2020 for \$351 million in cash, giving a clean event date (2020Q1) inside the sample. Using an ASIN-by-quarter panel of the Amazon US toothpaste market (2018Q1--2022Q3, 159 products), I estimate three demand models via the Berry (1994) nested-logit linearisation that are identical in every respect except the nest assignment: M1 imposes no nesting (plain logit), M2 nests by brand, and M3 nests by K=5 CLIP clusters. The price coefficient is calibrated to the meta-analytic mean own-price elasticity of $-2.62$ for consumer packaged goods (Bijmolt, van Heerde, and Pieters 2005), the remaining parameters are estimated by concentrated OLS with quarter fixed effects, and the nesting parameter $\rho$ is profiled by grid search. Marginal costs are recovered from the Bertrand first-order conditions, and the merger is simulated by recomputing Nash equilibrium prices under pre- and post-merger ownership at fixed pre-merger costs.
